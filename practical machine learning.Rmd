...
The data set is down to 93 variables. The next step is to discard columns that won't help prediction. The timestamp is unlikely to help, neither are the skewness, window or kurtosis variables. 
```{r}
...
dim(temp)
```
Now there are 53 variables to feed into the machine learning algorithm, or 52 when "classe" is excluded. This should improve the speed in which the model is built. 
...
The model appears to be highly accurate with only a few cases mispredicted. Specifically, there were 11 cases of A miscategorized as B, 16 cases of C to D, and 13 cases of D to C. However, this only a few among over a thousand.
The accuracy reported by the confusionMatrix function as performed on the testing data is 99.15%. This is a very high level of accuracy leaving only a 0.85% out of sample error. This level of accuracy is so high that it makes me concerned about overfitting, however that is beyond the scope of this project. 
